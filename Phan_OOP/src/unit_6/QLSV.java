...
public class QLSV extends SuLyNgoaiLe{
	private String name;
	private int age;
	...
	public void nhap(){
		...
		while(true) {       // nhap vao ten rong
			System.out.print(" nhap ten sinh vien : ");
			name = sc.nextLine();
			try {
				SuLyNgoaiLe.chkHoTen(name);
				break;
			}catch (SuLyNgoaiLe e) {
				System.err.println(e);
			}
		}
		while(true) {    // nhap vao tuoi <0 && >100
			System.out.print(" nhap tuoi sinh vien :");
			age = Integer.parseInt(sc.nextLine());
			try {
				SuLyNgoaiLe.chkTuoi(age);
				break;
			}catch(SuLyNgoaiLe e) {
				System.err.println(e);
			}
		...
		while(true) {    // id sinh vien rong
		System.out.print(" nhap id sinh vien : ");
		id = sc.nextLine();
		try {
			SuLyNgoaiLe.chkmasv(id);
			break;
		}catch(SuLyNgoaiLe e){
			System.err.println(e);
		}
		...
		while(true) {       //  diem TB <0 va >10
			System.out.print(" nhap diem trung binh : ");
			gpa = Double.parseDouble(sc.nextLine());
			try {
				SuLyNgoaiLe.chkDiem(gpa);
				break;
			}catch(SuLyNgoaiLe e) {
				System.err.println(e);
			}
	...
	public void xuat() {
			System.out.printf("\n\t\t %-20s %-10s %-20s %-10s %-10s",name ,id ,address,gpa,age );
	}
				...
				case 2 :{
					...
					for(int i =0; i<danhsachsv.size(); i++) {
						if(sua.equalsIgnoreCase(danhsachsv.get(i).getId())) {   // neu thoa dk moi cho chay
							...
							System.out.printf("\n\t\t%-20s %-10s %-20s %-10s 5-10s","Ten sinh vien","ID","Dia Chi","DiemTB", "Tuoi");
				...
				case 3 :{
					...
					for(int i =0; i<danhsachsv.size(); i++) {
						if(xoa.equalsIgnoreCase(danhsachsv.get(i).getId())) {   // neu thoa dk moi cho chay
							...
							System.out.printf("\t\t====DANH SACH SINH VIEN SAU KHI DA SUA====");
							System.out.printf("\n\t\t%-20s %-10s %-20s %-10s %-10s ","Ten sinh vien","ID","Dia Chi","DiemTB","Tuoi");
							for(int j=0; j<danhsachsv.size(); j++) {
								danhsachsv.get(j).xuat();	
							}
						}
